...
import { SCORING_WEIGHTS, SECTION_IDS, MAX_COUNTS } from "../types";
...
const Dashboard: React.FC<DashboardProps> = ({ records }) => {
  const stats = useMemo(() => {
    ...
    const avgScore =
      scoresFull.length > 0
        ? scoresFull.reduce((a, b) => a + b, 0) / scoresFull.length
        : 0;
    ...
    const getNormalizedAvg = (
      subSections: {
        id: string;
        maxCount: number;
        getValue: (r: PracticeRecord) => number;
      }[],
      weight: number
    ) => {
    ...
    const avg_L_Long = getNormalizedAvg(
      [
        {
          id: SECTION_IDS.L_LC1,
          maxCount: MAX_COUNTS.listening.longConversation1,
          getValue: (r) => r.inputs.listening.longConversation1,
        },
        {
          id: SECTION_IDS.L_LC2,
          maxCount: MAX_COUNTS.listening.longConversation2,
          getValue: (r) => r.inputs.listening.longConversation2,
        },
      ],
    ...
    const avg_L_Pass = getNormalizedAvg(
      [
        {
          id: SECTION_IDS.L_P1,
          maxCount: MAX_COUNTS.listening.passage1,
          getValue: (r) => r.inputs.listening.passage1,
        },
        {
          id: SECTION_IDS.L_P2,
          maxCount: MAX_COUNTS.listening.passage2,
          getValue: (r) => r.inputs.listening.passage2,
        },
      ],
    ...
    const avg_L_Lect = getNormalizedAvg(
      [
        {
          id: SECTION_IDS.L_LEC1,
          maxCount: MAX_COUNTS.listening.lectures1,
          getValue: (r) => r.inputs.listening.lectures1,
        },
        {
          id: SECTION_IDS.L_LEC2,
          maxCount: MAX_COUNTS.listening.lectures2,
          getValue: (r) => r.inputs.listening.lectures2,
        },
        {
          id: SECTION_IDS.L_LEC3,
          maxCount: MAX_COUNTS.listening.lectures3,
          getValue: (r) => r.inputs.listening.lectures3,
        },
      ],
    ...
    const avg_R_Careful = getNormalizedAvg(
      [
        {
          id: SECTION_IDS.R_CR1,
          maxCount: MAX_COUNTS.reading.carefulReading1,
          getValue: (r) => r.inputs.reading.carefulReading1,
        },
        {
          id: SECTION_IDS.R_CR2,
          maxCount: MAX_COUNTS.reading.carefulReading2,
          getValue: (r) => r.inputs.reading.carefulReading2,
        },
      ],
    ...
    const getSimpleAvg = (
      ...
      return (
        attempts.reduce((acc, r) => acc + getValue(r) * weight, 0) /
        attempts.length
      );
    };

    const avg_R_Banked = getSimpleAvg(
      SECTION_IDS.R_BC,
      (r) => r.inputs.reading.bankedCloze,
      SCORING_WEIGHTS.reading.bankedCloze
    );
    const avg_R_Match = getSimpleAvg(
      SECTION_IDS.R_MAT,
      (r) => r.inputs.reading.matching,
      SCORING_WEIGHTS.reading.matching
    );
    ...
    const avg_Write = getSimpleAvg(
      SECTION_IDS.W_WRIT,
      (r) => r.inputs.others.writing,
      SCORING_WEIGHTS.others.multiplier
    );
    const avg_Trans = getSimpleAvg(
      SECTION_IDS.T_TRANS,
      (r) => r.inputs.others.translation,
      SCORING_WEIGHTS.others.multiplier
    );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-12 gap-y-4">
          ...
          <div>
            ...
            <SectionStat
              ...
              value={stats.granular.r_careful}
            />
          </div>
          <div>
